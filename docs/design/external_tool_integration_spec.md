...
Both `manifest.json` and `jobs.json` include a `schema_version` field. Tools **must**:

1. Write the current schema version when creating files
2. Check schema version when reading files
3. Handle older schema versions gracefully (if possible) or error clearly
...
2. New required fields require a minor version bump
3. Removed or renamed fields require a major version bump
...
| `manifest.json` creation | §6.1 | ⚠️ Partial | See C.2.1, C.2.2 |
| `jobs.json` creation | §6.2 | ⚠️ Partial | See C.2.3 |
| Configuration hierarchy | §7 | ✅ Compliant | |
| Stage abstraction | §8 | ✅ Compliant | |
| Runner abstraction | §9 | ✅ Compliant | |
| Exit codes | §10 | ⚠️ Unverified | See C.2.4 |
| SLURM dependency handling | §4.5 | ✅ Compliant | |
| Resubmission with archival | §4.6 | ✅ Compliant | |

### C.2 Outstanding items

#### C.2.1 Missing `schema_version` in manifest.json

**Specification reference:** §6.1.3

**Current state:** The `manifest.json` generated by BayesEoR does not include a `schema_version` field.

**Required change:** Add `"schema_version": "1.0"` to manifest generation in `setup.py`.

**Priority:** Medium — Required before spec v1.0 finalisation.

---

#### C.2.2 Missing `tool` field in manifest.json
...
#### C.2.3 Missing `schema_version` in jobs.json

**Specification reference:** §6.2.3

**Current state:** The `jobs.json` generated by BayesEoR does not include a `schema_version` field.

**Required change:** Add `"schema_version": "1.0"` to jobs.json generation in `submit.py`.

**Priority:** Medium — Required before spec v1.0 finalisation.

---

#### C.2.4 Exit codes not audited
...
#### C.2.5 `--results-root` argument not verified
...
1. **Schema version fields (C.2.1, C.2.3)** — Simple additions with no behavioural change
2. **Tool field (C.2.2)** — Simple addition with no behavioural change
3. **`--results-root` verification (C.2.5)** — Quick audit
4. **Exit code audit (C.2.4)** — May require minor refactoring
